...
#-----------------------------------------
catch_file_ex  <-  file.path(data_dir, "O_Catch - sole.csv")    #"O_ICES_Catch_10.csv" # name of file containing "stock", "yr", "ct", and optional "bt"
id_file_ex     <-  file.path(data_dir, "O_ID - sole.csv")  #"O_ICES_ID_11.csv"    # name of file containing stock-specific info and settings for the analysis

outfile     <- paste("Out_",format(Sys.Date(),format="%B%d%Y_"),id_file_ex,sep="") # default name for output file
...
stocks <- c("HAKE_sim")     #c("cod-2224") #c("tur-kask")   #c("cod-scow") # c("MULLBAR_BA") #c("CORYHIP_SA")  #   c("Tur_BS")  #  #   "sal-2231"    #c("sol-iris") # c("cod-2224")   
...
select.yr    <- 20 # option to display F, B, F/Fmsy and B/Bmsy for a certain year; default NA
...
# Read data
cdat_ex         <- read.csv(catch_file_ex, header=T, dec=".", stringsAsFactors = FALSE)
cinfo_ex        <- read.csv(id_file_ex, header=T, dec=".", stringsAsFactors = FALSE)
cat("Files", catch_file_ex, ",", id_file_ex, "read successfully","\n")

# Change selectivity
hake$sel50 <- 4.0
hake$sel95 <- 5.0

cdat <- data.frame("Stock"=rep(stocks[1], nrow(hake$data)), "yr"=hake$data$year, "ct"=hake$data$catch)
cinfo <- cinfo_ex
  cinfo$Region <- "Sim"
  cinfo$Subregion <- "Sim"
  cinfo$Stock <- stocks[1]
  cinfo$Name <- stocks[1]
  cinfo$EnglishName <- stocks[1]
  cinfo$ScientificName <- stocks[1]
  cinfo$MinOfYear <- min(cdat$yr)
  cinfo$MaxOfYear <- max(cdat$yr)
  cinfo$StartYear <- min(cdat$yr)
  cinfo$EndYear <- max(cdat$yr)
  cinfo$stb.low <- 0.01
  cinfo$stb.hi <- 0.99
  cinfo$int.yr <- NA
  cinfo$intb.low <- NA
...
if(is.na(stocks[1])==TRUE){
  ...
  # stocks         <- sort(as.character(cinfo$Stock)) # Analyze stocks in alphabetic order
   stocks         <- as.character(cinfo$Stock[cinfo$Subregion=="Adriatic Sea"]) # Analyze stocks in Region
}

...
devtools::install_github("merrillrudd/catchMSY", dependencies=TRUE)
library(catchMSY)
...
sim_dir <- file.path(main_dir, "sim_results")
...
plot(OM_new$ft, ylim=c(0,max(OM_new$ft)*1.2), type="o", pch=19, xaxt="n")
mtext(side=2, "Fishing Mortality", line=3, font=2)
plot(OM_new$ML, ylim=c(0, max(OM_new$ML)*1.2), type="o", pch=19)
mtext(side=2, "Mean length", line=3, font=2)
mtext(side=1, "Year", line=3, font=2)
